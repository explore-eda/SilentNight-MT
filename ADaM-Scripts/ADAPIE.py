...
APIE_path = '/Users/haoxiang/Desktop/SilentNight-MT/transformed_files/Transformed_APIE.xlsx'
spec_path = '/Users/haoxiang/Desktop/SilentNight-MT/XYZ_ADaM_Spec_v1.xlsx'

passed_sheet_name = 'ADAPIE'

APIE_data = pd.read_excel(APIE_path, sheet_name='Sheet1')
spec_data = pd.read_excel(spec_path, sheet_name=passed_sheet_name)

transformed_data = pd.DataFrame(index=APIE_data.index)

transformed_data['APID'] = APIE_data['APID']
transformed_data['DOMAIN'] = passed_sheet_name
transformed_data['RSUBJID'] = APIE_data['RSUBJID']
transformed_data['SREL'] = APIE_data['SREL']
transformed_data['IESEQ'] = APIE_data['IESEQ']
transformed_data['IECAT'] = APIE_data['IECAT']
transformed_data['PARAM'] = APIE_data['IETEST']
transformed_data['PARAMCD'] = APIE_data['IETESTCD']
transformed_data['AVAL'] = APIE_data['IESTRESC']

transformed_data['IEDT'] = pd.to_datetime(APIE_data['IEDTC'], errors='coerce').dt.strftime('%m/%d/%Y')

transformed_data['INCLFL'] = APIE_data['IECAT'].apply(lambda x: 'Y' if x == 'Inclusion' else None)
transformed_data['EXCLFL'] = APIE_data['IECAT'].apply(lambda x: 'Y' if x == 'Exclusion' else None)
...
domain_names = {passed_sheet_name: "Analysis Dataset of Associated Person of Inclusion and Exclusion Criteria"}
required_columns = spec_data['Variable Name'].dropna().tolist()
