...
DI_path = '/Users/haoxiang/Desktop/SilentNight-MT/transformed_files/Transformed_DI.xlsx'
spec_path = '/Users/haoxiang/Desktop/SilentNight-MT/XYZ_ADaM_Spec_v1.xlsx'

passed_sheet_name = 'ADDI'

DI_data = pd.read_excel(DI_path, sheet_name='Sheet1')
spec_data = pd.read_excel(spec_path, sheet_name=passed_sheet_name)

transformed_data = pd.DataFrame(index=DI_data.index)

transformed_data['SPDEVID'] = DI_data['SPDEVID']
transformed_data['DISEQ'] = DI_data['DISEQ']
transformed_data['PARAM'] = DI_data['DIPARM']
transformed_data['PARAMCD'] = DI_data['DIPARMCD']
transformed_data['AVAL'] = DI_data['DIVAL']
...
domain_names = {passed_sheet_name: "Analysis Dataset of Device Identifiers"}
required_columns = spec_data['Variable Name'].dropna().tolist()
