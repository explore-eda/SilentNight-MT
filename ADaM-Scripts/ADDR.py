...
DR_path = '/Users/haoxiang/Desktop/SilentNight-MT/transformed_files/Transformed_DR.xlsx'
spec_path = '/Users/haoxiang/Desktop/SilentNight-MT/XYZ_ADaM_Spec_v1.xlsx'

passed_sheet_name = 'ADDR'

DR_data = pd.read_excel(DR_path, sheet_name='Sheet1')
spec_data = pd.read_excel(spec_path, sheet_name=passed_sheet_name)

transformed_data = pd.DataFrame(index=DR_data.index)

transformed_data['STUDYID'] = DR_data['STUDYID']
transformed_data['USUBJID'] = DR_data['USUBJID']
transformed_data['SPDEVID'] = DR_data['SPDEVID']
...
domain_names = {passed_sheet_name: "Analysis Dataset of Device Subject Relationships"}
required_columns = spec_data['Variable Name'].dropna().tolist()
